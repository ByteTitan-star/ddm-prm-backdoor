...
def fine_pruning(args):
    """Fine-pruning: zero low-contribution activations on a small clean set then fine-tune."""
    ...
    clean_dataset, _ = build_backdoor_dataset(0, args, is_train=False) 
    ...
    with open(args.output_path, "w") as outs:
        for index in range(pruning_mask.shape[0]):
            ...
            if args.model == 'myresnet18':
                ...
                assert isinstance(model, ResNet), 'Model should be ResNet-18'
                ...
                for name, module in net_pruned._modules.items():
                    if "layer3" in name:
                        ...
                        module.weight.data = model.linear6.weight.data.reshape(-1, 64, 16)[:, pruning_mask].reshape(512, -1)
            ...
            if isinstance(clean, torch.Tensor): clean = clean.item()
            if isinstance(bd, torch.Tensor): bd = bd.item()
            
            acc_clean_list.append(clean)
            acc_bd_list.append(bd)
            index_list.append(index)

            outs.write("%d %0.4f %0.4f\n" % (index, clean, bd))
            print(f'Pruned {num_pruned} filters | Clean Accuracy: {clean:.4f} | Backdoor ASR: {bd:.4f}')

    print("Pruning finished. Generating plot...")
    
    plt.figure(figsize=(10, 6))
    
    plt.plot(index_list, acc_clean_list, label='CA (Clean Accuracy)', color='blue', linewidth=1.5)
    
    plt.plot(index_list, acc_bd_list, label='ASR (Attack Success Rate)', color='orange', linewidth=1.5)
    
    plt.xlabel("Number of Filters Pruned")
    plt.ylabel("Percentage(%)")
    plt.title(f"Fine-Pruning Results: {args.data_set} - {args.attack_mode}")
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    
    plot_path = args.output_path.replace('.txt', '.png')
