...
def strip(args):
    """STRIP: entropy-based trojan input detection; runs multiple rounds and saves histograms."""
    ...
    try:
        trojan_arr = np.array(lists_entropy_trojan)
        benign_arr = np.array(lists_entropy_benign)

        bins = 30

        plt.figure(figsize=(6.0, 4.0), dpi=200)

        plt.hist(benign_arr, bins=bins, density=True, alpha=0.6, label='clean', edgecolor='none')
        plt.hist(trojan_arr, bins=bins, density=True, alpha=0.6, label='backdoor', edgecolor='none')

        plt.xlabel('Entropy')
        plt.ylabel('Probability')
        plt.title(f'STRIP Entropy Distribution ({args.data_set}, mode={args.attack_mode})')
        plt.legend(loc='upper right', fontsize='small')


        hist_png = os.path.join(args.output_dir, f'strip_entropy_{args.data_set}_{args.attack_mode}.png')
        hist_pdf = os.path.join(args.output_dir, f'strip_entropy_{args.data_set}_{args.attack_mode}.pdf')
        plt.tight_layout()
        ...
        print(f"Histogram saved to: {hist_pdf}")
    except Exception as e:
        print("Error when plotting histogram:", e)
