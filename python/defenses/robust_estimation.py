...
class BeingRobust(EmpiricalCovariance,BaseEstimator):
    ...
    def fit(self, X, y=None) -> 'BeingRobust':
        ...
        """
        arr = np.array(X)
        print("合法二维数据集" if arr.ndim == 2 and arr.shape[0] > 0 and arr.shape[1] > 0 else "不是合法二维数据集")
        X = validate_data(self, X, ensure_min_samples=1, estimator='BeingRobust')
