...
def get_result(model, dataset, poi_idx):
    poi_idx = np.array(poi_idx).ravel().astype(int).tolist()
...
def adjusted_outlyingness(series):
    ...
    return torch.tensor(_ao, device=series.device)
...
def run_asset(args):
    """ASSET: contrastive separation of poison vs clean losses with MLP scorer on statistics."""
    ...
    for epoch in tqdm(range(1)):
        ...
        for iters, (input_train, target_train, _, _, poi) in enumerate(tqdm(train_dataloader)):
            pos_img,pos_lab,poi = input_train.to(device), target_train.to(device), poi.to(device)
            idxs = random.sample(range(valid_size), min(batch_size,valid_size))
            neg_img = torch.stack([val_set[i][0] for i in idxs]).to(device)
            neg_lab = torch.tensor([val_set[i][1] for i in idxs], device=device)
            ...
            for _ in range(100):
                ...
                v_label = torch.ones(v_outputs.shape[0], device=device)
                ...
                v_label2 = torch.zeros(vn_outputs.shape[0], device=device)
